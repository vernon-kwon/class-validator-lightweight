...
export class MetadataStorage {
  ...
  addValidationSchema(schema: ValidationSchema): void {
    const validationMetadatas =
      new ValidationSchemaToMetadataTransformer().transform(schema);
  ...
  getTargetValidationMetadatas(
    ...
    const filteredForOriginalMetadatasSearch =
      this.validationMetadatas.get(targetConstructor) || [];
    const originalMetadatas = filteredForOriginalMetadatasSearch.filter(
      (metadata) => {
        if (
    ...
    const inheritedMetadatas = filteredForInheritedMetadatasSearch.filter(
      (metadata) => {
