...
import { convertToArray } from "utils/convert-to-array.util";
...
export class ValidationExecutor {
  ...
  execute(
    ...
    if (
      ...
      this.validatorOptions?.enableDebugMessages === true
    ) {
      console.warn(
    ...
    const groups = this.validatorOptions
      ...
      : undefined;
    const strictGroups =
      (this.validatorOptions && this.validatorOptions.strictGroups) || false;
    const always =
      (this.validatorOptions && this.validatorOptions.always) || false;
    ...
    const groupedMetadatas =
      this.metadataStorage.groupByPropertyName(targetMetadatas);
    ...
    Object.keys(groupedMetadatas).forEach((propertyName) => {
      ...
      if (
        ...
        metadatas.find(
          (metadata) => metadata.type === ValidationTypes.PROMISE_VALIDATION
        )
      ) {
    ...
    if (notAllowedProperties.length > 0) {
      if (this.validatorOptions && this.validatorOptions.forbidNonWhitelisted) {
        ...
        notAllowedProperties.forEach((property) => {
          const validationError: ValidationError = this.generateValidationError(
            object,
            object[property],
            property
          );
  ...
  private performValidations(
    ...
    if (
      value === undefined &&
      this.validatorOptions &&
      this.validatorOptions.skipUndefinedProperties === true
    ) {
      return;
    }
    ...
    if (
      value === null &&
      this.validatorOptions &&
      this.validatorOptions.skipNullProperties === true
    ) {
      return;
    }
      ...
      this.metadataStorage
        ...
        .forEach((customConstraintMetadata) => {
          ...
          const validatedSubValues = arrayValue.map((subValue: any) =>
            customConstraintMetadata.instance.validate(
              subValue,
              validationArguments
            )
          );
          const validationIsAsync = validatedSubValues.some(
            (validatedSubValue: boolean | Promise<boolean>) =>
          ...
          if (validationIsAsync) {
            ...
            const asyncValidatedSubValues = validatedSubValues.map(
              (validatedSubValue: boolean | Promise<boolean>) =>
                isPromise(validatedSubValue)
                  ? validatedSubValue
                  : Promise.resolve(validatedSubValue)
            );
            const asyncValidationIsFinishedPromise = Promise.all(
              asyncValidatedSubValues
            ).then((flatValidatedValues: boolean[]) => {
              const validationResult = flatValidatedValues.every(
          ...
          const validationResult = validatedSubValues.every(
            (isValid: boolean) => isValid
          );
          if (!validationResult) {
            const [type, message] = this.createValidationError(
      ...
      if (metadata.context) {
        ...
        if (metadata.type === ValidationTypes.CUSTOM_VALIDATION) {
          const customConstraints =
            this.metadataStorage.getTargetValidatorConstraints(
  ...
  private createValidationError(
    ...
    if (
      ...
      if (
        ...
        customValidatorMetadata.instance.defaultMessage instanceof Function
      ) {
        message =
          customValidatorMetadata.instance.defaultMessage(validationArguments);
      }
    }
